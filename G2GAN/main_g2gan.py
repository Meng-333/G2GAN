import torch
import os, argparse
from model.g2gan import G2GAN     # 本文提出的方法
...
def parse_args():
    ...
    parser.add_argument('--model_name', type=str, default='G2GAN',
                        choices=['DSSR', 'SwinIR', 'SRAGAN', 'SRDSRAN', 'DASRGAN', 'InfraFFN', 'IRSRMamba', 'G2GAN'], help='The type of model')
    parser.add_argument('--root_dir', type=str, default='/root/autodl-tmp/G2GAN/')
    parser.add_argument('--data_dir', type=str, default='/root/autodl-tmp/G2GAN/dataset/')
    parser.add_argument('--train_dataset', type=list, default=["Train"], choices=["AVMS", "FLIR", "DroneVehicle", "M3IR"],
                        help='The name of training dataset')
    parser.add_argument('--test_dataset', type=list,
                        default=["Test"],
                        help='The name of test dataset')
    parser.add_argument('--crop_size', type=int, default=512, help='Size of cropped HR image')
    ...
    parser.add_argument('--test_crop_size', type=int, default=512, help='Size of cropped HR image')
    parser.add_argument('--n_cpu', type=int, default=16, help='number of cpu threads to use during batch generation')
    parser.add_argument('--hr_height', type=int, default=512, help='size of high res. image height')
    parser.add_argument('--hr_width', type=int, default=512, help='size of high res. image width')
    ...
    parser.add_argument('--gradient_loss', type=bool, default=True, help='gradient_loss')
    parser.add_argument('--weight_gradient', type=float, default=1e-2, help='Loss weight for gradient loss')
...
def main():
    ...
    # test
    # 测试集下不同数据集逐个测试
    net.mfeNew_validateByClass(100, save_img=True, modelpath="./Result/G2GAN/model/generator_param.pkl")
